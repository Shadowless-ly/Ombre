    ...
    def maxSub(self, nums):
        ...
        for i in range(len(nums)):
            ...
            for j in range(i,len(nums)):
                current_total.append(sum(nums[i:j+1]))
            total.append(max(current_total))
        return max(total)
...
class Solution2():
    ...
    def maxSubRec(self, nums, head, tail):
        # 递归边界
        if (head==tail):
            return nums[head]
        ...
        # 递归分治部分
        middle = (head + tail) // 2
        maxHeadSum = self.maxSubRec(nums, head, middle)
        maxTailSum = self.maxSubRec(nums, middle, tail)
        ...
        while (i >= head):
            HeadBorderSum += a[i]
            i -= 1
            if (maxHeadBorderSum < HeadBorderSum):
                maxHeadBorderSum = HeadBorderSum
